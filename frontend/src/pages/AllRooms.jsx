...
const AllRooms = () => {
    ...
    const filterDestination=(room)=>{
        ...
        return room.property.city.toLowerCase().includes(destination.toLowerCase())
    }
    ...
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        <div>
            <div className='flex flex-col items-start text-left'>
                ...
                <p className='text-sm md:text-base text-gray-500/90 mt-2 max-w-174'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Obcaecati, tempora. Aperiam excepturi id ut laudantium iure placeat numquam illum animi praesentium minus quaerat explicabo illo, consectetur deserunt! Corrupti, esse ullam?</p>

            </div>
            {filteredRooms.map((room)=>(
                <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0'>
                    <img src={room.images[0]} alt="room-img" onClick={()=>{navigate(`/rooms/${room._id}`); scrollTo(0,0)}} title='View Room Details' className='max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer'/>
                    <div className='md:w-1/2 flex flex-col gap-2'>
                        <p className='text-gray-500'>{room.property.city}</p>
                        <p onClick={()=>{navigate(`/rooms/${room._id}`); scrollTo(0,0)}} className='text-gray-800 text-3xl font-playfair cursor-pointer'>{room.property.name}</p>
                        ...
                        <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                            {room.amenities.map((item,index)=>(
                                <div key={index} className='flex items-center gap-2 px-3 py-2 roundedlg bg-[#FSFSFF]/70'>
                        ...
                        {/* Room Price */}
                        <p className='text-xl font-medium text-gray-700'>Rs. {room.pricePerMonth}/month</p>
                    </div>
                </div>
            ))}
        </div>
